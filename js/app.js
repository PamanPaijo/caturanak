...
function handlePlayTap(hit) {
  ...
  // sentuh penanda -> jalankan langkah pemain
  if (hit && hit.isMarker && selected) {
    busy = true;
    const w = selected.wrap;
    const target = pieceAt(hit.r, hit.c);
    const captured = applyMove(gameBoard, selected.r, selected.c, hit.r, hit.c);
    clearMarkers();
    selected = null;
    const after = () => {
      movePieceMesh(w, hit.r, hit.c, () => {
    ...
    if (target) removePieceMesh(target, after); else after();
    return;
  }
  // sentuh bidak sendiri -> pilih
